<template>
  <div class = "ue">
    ...
    <div class = "article-handle">
      <div class = "publish">
        <button @click = "save()" :disabled = "wating.disabled" class = "true-publish">{{wating.info.s}}</button>
        <button @click = "publish()" :disabled = "wating.disabled" class = "true-publish">{{wating.info.p}}</button>
      </div>
    </div>
  ...
  export default {
    ...
    data() {
      return {
        ...
        articleTags:[
          "CSS","html","JavaScript","nodejs","Vue","webpack","react","work","life","other"
        ]
      }
    },
    mounted() {
    ...
    methods: {
      save() {// 新建草稿
        if(this.validate()){
          ...
          this.wating = {disabled: true,info: {p: "发表文章",s: "保存中..."}}
          
          webHttp.request({
            url: '/api/draftsCreate',
            method: 'POST',
            data: {
              title: _title,
              abstract: _abstract,
            ...
            callback: (res) => {
              ...
              if(res.success){
                ...
                that.wating = {disabled: false,info: {p: "发表文章",s: "保存草稿"}}
      ...
      publish: function(){//发表文章
        if(this.validate()){
          let _title = this.articleInfo.title,
            ...
            this.wating = {disabled: true,info: {p: "发表中...",s: "保存草稿"}}
